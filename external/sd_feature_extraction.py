...
from external.cleandift.src.layers import FeedForwardBlock, FourierFeatures, Linear, MappingNetwork
from external.cleandift.src.min_sd15 import SD15UNetModel
from external.cleandift.src.min_sd21 import SD21UNetModel
        ...
        if self.use_adapters:
            ...
            if not self.mapping is None:
                map_cond: Float[torch.Tensor, "(B N_T) ..."] = self.mapping(
                    self.time_in_proj(
                        self.time_emb(
                            einops.rearrange(t, "B N_T -> (B N_T) 1").to(dtype=x.dtype, device=device)
                            / self.t_max_model
                        )
                    )
                )
        ...
        if use_base_model:
            ...
            if t is None:
                ...
                if feat_key is not None:
                    ...
                    return {
                        key: einops.rearrange(self.adapters[key](feats[key], cond=map_cond), "B H W D -> B D H W")
